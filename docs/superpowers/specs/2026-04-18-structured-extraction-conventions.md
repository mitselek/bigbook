...
- **Chapter/story/appendix title** — the "real" heading the reader sees. Emit as a `heading` block. Example: "HOW IT WORKS", "BILL'S STORY".
- **Subtitles** accompanying story titles (the "deck" lines beneath, e.g. "She hid her bottles in clothes hampers..."). Emit as a single **paragraph** block when structurally a prose description, OR as its own heading if typography suggests a lower-level heading. Agent's call; document in `.md`.
