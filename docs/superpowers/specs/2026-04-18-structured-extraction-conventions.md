...
- **Multi-hyphen compounds at line breaks** — compounds with three-plus hyphenated parts (`life-and-death`, `face-to-face`, `ten-dollar-a-week`) can split at the middle hyphen, and the middle word (`and`, `to`) is never in the allowlist. Extra rule: if the out-buffer already ends with **at least two** intermediate-word-with-hyphen patterns (require two trailing hyphens in the current token — regex `^\w+-\w+-$` or similar "has ≥ 2 existing hyphens"), preserve the trailing hyphen on a cross-line `-` join regardless of the allowlist. Stricter than the Wave 5 original (`-\w+-$`) to avoid the false positive `get-to-` + `gether` → `get-to-gether` (should be `get-together`). The stricter rule still catches `life-and-` + `death`, `face-to-` + `face`, `ten-dollar-a-` + `week`.
