...
Expected: exit 0. If `manifest.ts` has TypeScript errors (unknown type, unquoted string containing a single quote, etc.), the script's emitter has a bug — investigate in `scripts/bootstrap-mock-content.ts` emitter and re-run P6.1.

- [ ] **Step 4: Dry-run the hard-invariant hook against the new content**

```bash
# Fake a staged list including one of the generated files
npx tsx scripts/hard-invariant.ts src/content/et/billi-lugu.md src/content/en/billi-lugu.md
```
...
Generated by scripts/bootstrap-mock-content.ts on 2026-04-15.
